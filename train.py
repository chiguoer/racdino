...
def main():
    ...
    parser.add_argument('--resume_from', help='the checkpoint file to resume from')  # 添加这行
    ...
    # Force evaluation every epoch so metrics (e.g., mAP/NDS) are logged each round
    if 'eval_config' in cfgs and isinstance(cfgs.eval_config, dict):
        cfgs.eval_config['interval'] = 1
    if args.resume_from:
        cfgs.resume_from = args.resume_from  # 使用正确的变量名 cfgs
        logging.info(f'覆盖恢复路径: {cfgs.resume_from}')
