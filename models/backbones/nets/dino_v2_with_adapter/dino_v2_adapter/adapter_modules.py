...
from ..dino_v2.layers.drop_path import DropPath
...
def deform_inputs(x, c2_shape=None, c3_shape=None, c4_shape=None):
    ...
    reference_points = get_reference_points([(H2, W2), (H3, W3), (H4, W4)], x.device)
...
class ConvFFN(nn.Module):
    ...
    def forward(self, x, H, W, spm_shapes=None):
        x = self.fc1(x)
        x = self.dwconv(x, H, W, spm_shapes=spm_shapes)
        x = self.act(x)
        x = self.drop(x)
        x = self.fc2(x)
        x = self.drop(x)
...
class DWConv(nn.Module):
    ...
    def forward(self, x, H, W, spm_shapes=None):
        """
        Args:
            x: [B, N, C] 拼接的 c2, c3, c4 特征
            H, W: 用于计算的基础尺寸（当 spm_shapes 为 None 时使用）
            spm_shapes: 可选，SPM 各层的实际形状 [(H2, W2), (H3, W3), (H4, W4)]
        """
        B, N, C = x.shape
        
        if spm_shapes is not None:
            # 使用精确的 SPM 形状
            (H2, W2), (H3, W3), (H4, W4) = spm_shapes
            n1 = H2 * W2  # c2 的长度
            n2 = H3 * W3  # c3 的长度
            n3 = H4 * W4  # c4 的长度
            
            x1 = x[:, 0:n1, :].transpose(1, 2).view(B, C, H2, W2).contiguous()
            x2 = x[:, n1:n1+n2, :].transpose(1, 2).view(B, C, H3, W3).contiguous()
            x3 = x[:, n1+n2:n1+n2+n3, :].transpose(1, 2).view(B, C, H4, W4).contiguous()
        else:
            # 回退到原始计算方式（可能不准确）
            n1 = 4 * H * W  # H*2 x W*2
            n2 = H * W      # H x W
            n3 = (H // 2) * (W // 2)  # H//2 x W//2
            
            x1 = x[:, 0:n1, :].transpose(1, 2).view(B, C, H*2, W*2).contiguous()
            x2 = x[:, n1:n1+n2, :].transpose(1, 2).view(B, C, H, W).contiguous()
            x3 = x[:, n1+n2:n1+n2+n3, :].transpose(1, 2).view(B, C, H//2, W//2).contiguous()
...
class Extractor(nn.Module):
    ...
    def forward(self, query, reference_points, feat, spatial_shapes, level_start_index, H, W, spm_shapes=None):
        ...
        def _inner_forward(query, feat):
            ...
            with autocast(enabled=False):
                ...
                if self.with_cffn:
                    query = query + self.drop_path(self.ffn(self.ffn_norm(query), H, W, spm_shapes=spm_shapes))
                return query
...
class InteractionBlockWithCls(nn.Module):
    ...
    def forward(self, x, c, cls, blocks, deform_inputs1, deform_inputs2, H_adapt, W_adapt, spm_shapes=None):
        """
        Args:
            spm_shapes: SPM 各层的实际形状 [(H2, W2), (H3, W3), (H4, W4)]，用于 DWConv 中正确分割特征
        """
        x = self.injector(query=x, reference_points=deform_inputs1[0],
                          feat=c, spatial_shapes=deform_inputs1[1],
                          level_start_index=deform_inputs1[2])
        ...
        c = self.extractor(query=c, reference_points=deform_inputs2[0],
                           feat=x, spatial_shapes=deform_inputs2[1],
                           level_start_index=deform_inputs2[2], H=H_adapt, W=W_adapt, spm_shapes=spm_shapes)
