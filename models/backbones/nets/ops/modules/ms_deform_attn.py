...
class MSDeformAttn(nn.Module):
    ...
    def forward(self, query, reference_points, input_flatten, input_spatial_shapes, input_level_start_index,
        ...
        # 动态调整 im2col_step 以满足 batch % im2col_step == 0 的要求
        im2col_step = self.im2col_step
        if value.shape[0] % im2col_step != 0:
            # 使用 GCD 找到最大的满足条件的 step
            im2col_step = math.gcd(value.shape[0], im2col_step)
            if im2col_step < 1: im2col_step = 1
