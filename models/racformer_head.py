...
class RaCFormer_head(DETRHead):
    ...
    def generate_points(self):
        """
        生成圆形线性倍增分布的查询初始化点（符合RaCFormer论文）
        - 使用极坐标系统 (theta, distance)
        - 从内圈到外圈，查询数量线性增加
        - 论文: "ensure a linear increase in the number of queries from inner to outer circles"
        
        例如：如果num_query=900, num_clusters=5
        - 圆1（最内）：60个查询
        - 圆2：120个查询
        - 圆3：180个查询
        - 圆4：240个查询
        ...
        assert theta_d.shape[0] == self.num_query, \
            f"生成的查询数量 {theta_d.shape[0]} 与期望的 {self.num_query} 不匹配"
